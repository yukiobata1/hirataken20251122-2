...
def main():
    ...
    # Optimizer settings (based on test_algorithms.py results)
    # Test results on synthetic problem:
    #   - Momentum: 99.3% χ² reduction (best)
    #   - Nesterov: 99.3% χ² reduction
    #   - Simple: 95.6% χ² reduction
    #   - Adaptive LR: reduced performance (too conservative)
    # However, real LAMMPS may behave differently - both options available

    method = 'momentum'     # Update method: 'simple', 'momentum', 'nesterov'
    beta = 0.9              # Momentum coefficient (0.9 is optimal)
    alpha = 0.3             # Learning rate

    # Adaptive learning rate (optional safety mechanism)
    # Enable if you see χ² oscillating or increasing
    adaptive_lr = False     # False: trust momentum (faster convergence)
                            # True: add safety net (more conservative)
    alpha_min = 0.05        # Minimum learning rate (if adaptive_lr=True)
    alpha_max = 0.5         # Maximum learning rate (if adaptive_lr=True)
    ...
    print(f"Update method: {method.upper()}")
    if method in ['momentum', 'nesterov']:
        print(f"Momentum coefficient β: {beta}")
    print(f"Learning rate α: {alpha} (fixed)")
    if adaptive_lr:
        print(f"Adaptive learning rate: ENABLED (range: {alpha_min} - {alpha_max})")
    else:
        print(f"Adaptive learning rate: DISABLED (momentum provides stability)")
    ...
    # Note: alpha is already initialized above (line 322)
    ...
    for iteration in range(1, max_iter + 1):
        ...
        print(f"Current learning rate α = {alpha:.4f}")
        ...
        if adaptive_lr and len(chi2_history) > 1:
            ...
            if delta_chi2 > 0:
                ...
                print(f"  → χ² increased, reducing α to {alpha:.4f}")
            elif delta_chi2 < 0 and abs(delta_chi2) > 10:
                # χ² decreased significantly - can increase learning rate slightly
                alpha = min(alpha * 1.05, alpha_max)
                print(f"  → χ² decreased significantly, increasing α to {alpha:.4f}")

        # 7. Plot current results
        ...
        # Apply weighted updates using PARTIAL g(r) with momentum
        ...
        U_ep_GaGa, velocity_GaGa = update_ep(
            r_grid, g_GaGa_on_grid, g_exp_on_grid, U_ep_GaGa,
            alpha=alpha * w_GaGa_norm, T=T, max_amp=max_amp, sigma_smooth=sigma_smooth,
            momentum=velocity_GaGa, beta=beta, method=method
        )
        U_ep_InIn, velocity_InIn = update_ep(
            r_grid, g_InIn_on_grid, g_exp_on_grid, U_ep_InIn,
            alpha=alpha * w_InIn_norm, T=T, max_amp=max_amp, sigma_smooth=sigma_smooth,
            momentum=velocity_InIn, beta=beta, method=method
        )
        U_ep_GaIn, velocity_GaIn = update_ep(
            r_grid, g_GaIn_on_grid, g_exp_on_grid, U_ep_GaIn,
            alpha=alpha * w_GaIn_norm, T=T, max_amp=max_amp, sigma_smooth=sigma_smooth,
            momentum=velocity_GaIn, beta=beta, method=method
        )
