...
def update_ep(r, g_sim, g_exp, U_ep_old, alpha=0.3, T=423.15, max_amp=1.0, sigma_smooth=2,
              momentum=None, beta=0.9, method='momentum'):
    """
    ...
    momentum : array or None
        Previous velocity/momentum term (for momentum method)
    beta : float
    ...
    velocity : array
        Updated velocity/momentum (for next iteration)
    """
    ...
    if method == 'simple':
        # Simple gradient descent (original method)
        U_ep_new = U_ep_old + gradient
        velocity = gradient  # Store for reference

    elif method == 'momentum':
        # Momentum method (reduces oscillations)
        # v_{t+1} = β * v_t + gradient
        # U_{t+1} = U_t + v_{t+1}
        ...
        # v_{t+1} = β * v_t + gradient
        # U_{t+1} = U_t + β * v_{t+1} + gradient
        velocity = beta * velocity + gradient
        U_ep_new = U_ep_old + beta * velocity + gradient

    else:
        raise ValueError(f"Unknown method: {method}. Use 'simple', 'momentum', or 'nesterov'")
    ...
    if sigma_smooth > 0:
        ...
        # Also smooth velocity to maintain consistency
        velocity = gaussian_filter1d(velocity, sigma=sigma_smooth)

    return U_ep_new, velocity
